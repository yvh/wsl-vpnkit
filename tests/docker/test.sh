...
mock_env="$common_env GVFORWARDER_PATH=/tests/mock-vm MOCK_LOG_DIR=$log_dir"

echo "case: default startup and SIGTERM cleanup"
rm -f "$log_dir/vm.log"
start_background "$log_dir/default.log" wsl-vpnkit
wait_for "wsltap was not created" interface_exists wsltap
wait_for "gvforwarder did not start" process_exists gvforwarder
wait_for "gvproxy-windows.exe did not start" process_exists gvproxy-windows.exe
wait_for "diagnostics did not pass" log_contains "$log_dir/default.log" \
    "check: all diagnostic checks passed"
ps -eo args= | grep -E -- '[g]vforwarder' >/dev/null || fail "gvforwarder did not start"
ps -eo args= | grep -E -- '[g]vproxy-windows.exe' >/dev/null || fail "gvproxy-windows.exe did not start"
vm_pid=$(find_process_pid 'gvforwarder') ||
    fail "could not find gvforwarder process"
gvproxy_pid=$(find_process_pid 'gvproxy-windows.exe') ||
    fail "could not find gvproxy-windows.exe process"
...
assert_process_stopped "$vm_pid" gvforwarder
assert_process_stopped "$gvproxy_pid" gvproxy-windows.exe
assert_clean

echo "case: real gvforwarder-owned DHCP lease with real gvproxy"
...
ip -4 addr show dev realdhcptap | grep -q -- "192.168.127.2/24" ||
    fail "real gvforwarder did not configure the DHCP local IP"
ip -4 route show default |
    grep -q -- "default via 192.168.127.1 dev realdhcptap" ||
    fail "real gvforwarder did not configure the DHCP default route"
...
real_vm_pid=$(find_process_pid 'gvforwarder') ||
    fail "could not find real gvforwarder process"
real_gvproxy_pid=$(find_process_pid 'gvproxy-windows.exe') ||
    fail "could not find real gvproxy-windows.exe process"
stop_background "$log_dir/real-dhcp.log"
assert_process_stopped "$real_vm_pid" gvforwarder
assert_process_stopped "$real_gvproxy_pid" gvproxy-windows.exe
...
find_process_pid '/tests/mock-vm' >/dev/null ||
    fail "diagnostic failure stopped gvforwarder"
...
grep -q -- "-mac=5a:94:ef:e4:0c:ef" "$log_dir/vm.log" ||
    fail "configured DHCP MAC was not passed to gvforwarder"
stop_background "$log_dir/config-dhcp.log"
assert_clean

echo "case: gvforwarder-owned DHCP lease"
# shellcheck disable=SC2086
...
if env $common_env GVFORWARDER_PATH=/does-not-exist wsl-vpnkit >"$log_dir/missing-vm.log" 2>&1; then
    fail "missing VM executable was accepted"
fi
grep -q -- "GVFORWARDER_PATH \[/does-not-exist\] does not exist" "$log_dir/missing-vm.log" ||
...
if env $common_env GVFORWARDER_PATH="$non_executable_vm" wsl-vpnkit \
    >"$log_dir/non-executable-vm.log" 2>&1; then
    fail "non-executable VM path was accepted"
fi
grep -q -- "GVFORWARDER_PATH \[$non_executable_vm\] is not executable" \
...
if env $common_env GVFORWARDER_PATH=/usr/local/bin/gvforwarder \
...
mkdir -p "$space_dir"
cp /usr/local/bin/gvforwarder "$space_dir/temp vm"
cp /usr/local/bin/gvproxy-windows.exe "$space_dir/temp gvproxy"
# shellcheck disable=SC2086
start_background "$log_dir/space-paths.log" env \
    ...
    GVFORWARDER_PATH="$space_dir/temp vm" \
...
space_vm_pid=$(find_process_pid 'temp vm') ||
    fail "could not find gvforwarder process with a space-containing path"
space_gvproxy_pid=$(find_process_pid 'temp gvproxy') ||
    fail "could not find gvproxy-windows.exe process with a space-containing path"
stop_background "$log_dir/space-paths.log"
assert_process_stopped "$space_vm_pid" gvforwarder
assert_process_stopped "$space_gvproxy_pid" gvproxy-windows.exe
assert_clean
